            ...
            <section className="w-full max-w-md px-4 mt-4 flex-1">
              {mode === "find" &&
                filteredRides.map((ride) => (
                  <div
                    key={ride.id}
                    className="bg-black text-white rounded-xl mb-4 overflow-hidden shadow-lg"
                  >
                    {/* HEADER */}
...
function LazyRouteMap({ ride, isLoaded, buildRoute }) {
  ...
  if (!isLoaded) {
    return (
      <div className="h-[180px] flex items-center justify-center text-sm bg-gray-100 text-gray-500">
        Loading map...
      </div>
    );
  }
